...
class OutputBlock(nn.Module):
    ...
    def __init__(self, Ko, last_block_channel, channels, end_channel, n_vertex, gated_act_func, class_mat, use_class, n_pred, drop_rate):
        ...
        if use_class:
            self.fc1 = nn.Linear(channels[0] + class_mat.shape[0], channels[1])
        else:
            self.fc1 = nn.Linear(channels[0], channels[1])
        self.fc2 = nn.Linear(channels[1], end_channel * n_pred)
    ...
    def forward(self, x):
        ...
        # print("x_tc1.shape: ", x_tc1.shape)
        x_tc1_ln = self.tc1_ln(x_tc1.permute(0, 2, 3, 1))
        # print("x_tc1_ln.shape: ", x_tc1_ln.shape)
        if self.use_class:
            x_fc1 = self.fc1(torch.cat((x_tc1_ln, torch.repeat_interleave(self.class_mat.permute(1,0).unsqueeze(dim=0).unsqueeze(dim=0), repeats = x_tc1_ln.shape[0], dim=0)), -1))
        else:
        ...
        # x_out = torch.reshape(x_fc2, (x_fc2.shape[0], x_fc2.shape[1]*self.n_pred, x_fc2.shape[2], x_fc2.shape[3]))
        x_out = x_fc2
