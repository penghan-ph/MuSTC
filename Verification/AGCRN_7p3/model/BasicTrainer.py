...
class Trainer(object):
    def __init__(self, model, loss, optimizer, train_loader, val_loader, test_loader, classinfo,
    ...
    def val_epoch(self, epoch, val_dataloader):   #验证集
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                output = self.model(data, target, self.classinfo, teacher_forcing_ratio=0.)  #不使用标准答案
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            output = self.model(data, target, self.classinfo, teacher_forcing_ratio=teacher_forcing_ratio)
            if self.args.real_value:
                label = self.scaler.inverse_transform(label)
            loss = self.loss(output.cuda(), label)
            loss.backward()
    ...
    def train(self):
        ...
        self.test(self.model, self.args, self.test_loader, self.classinfo, self.scaler, self.logger)
    ...
    @staticmethod
    def test(model, args, data_loader, classinfo, scaler, logger, path=None):
        if path != None:
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(data_loader):
                ...
                output = model(data, target, classinfo, teacher_forcing_ratio=0)
                y_true.append(label)
